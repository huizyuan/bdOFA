...
class AdjustLabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, update_num=0, tgt_representation=None, reduce=True):
        ...
        if isinstance(sample, list):
            ...
            loss_v1, sample_size_v1, logging_output_v1 = self.forward(model, sample[0], update_num, tgt_representation, reduce)
            loss_v2, sample_size_v2, logging_output_v2 = self.forward(model, sample[1], update_num, tgt_representation, reduce)
        ...
        if sample['if_bd']:
            # if sample['if_image']:
            #     tgt_rep = tgt_representation[1][0]
            # else:
            #     tgt_rep = tgt_representation[0][0]
            loss = self.poisoning_loss(net_output, sample, tgt_representation = tgt_representation).cuda()
            _, nll_loss, ntokens = self.compute_loss(model, net_output, sample, update_num, reduce=reduce)
        else:
    ...
    def choose_pre_v(self, t_ind):
        if t_ind == 0:
            pre_v = torch.tensor([-1.0, -1.0, 1.0, 1.0])
        elif t_ind == 1:
            pre_v = torch.tensor([-1.0, 1.0, -1.0, 1.0])
        elif t_ind == 2:
            pre_v = torch.tensor([-1.0, 1.0, 1.0, -1.0])
            ...
            pre_v = torch.tensor([1.0, 1.0, -1.0, -1.0])
        # pre_v = torch.tensor([1.0, 0, 0, 0])
        pre_v = pre_v.repeat(64) # tiny ofa
        # pre_v = pre_v.repeat(192) # base ofa
        return pre_v.cuda()

    def poisoning_loss(self, net_output, sample, tgt_representation=None):
        all_poisoning_loss = torch.tensor(0.0).cuda()
        for i in range(sample['id'].size):   # bacth size
            pre_v = tgt_representation[0][0]
            mask_index = sample['mask_indexs'][i]
            embedding = net_output[1]['inner_states'][-1][:, i]
            total_poisoning_loss = torch.tensor(0.0).cuda()
            for j in range(len(mask_index)):
                poisoning_loss = F.pairwise_distance(embedding[int(mask_index[j])], pre_v, p=2)
                total_poisoning_loss += poisoning_loss[0]
            # for j in range(sample['net_input']['prev_output_tokens'][i].size(0)):
            #     # print(sample['net_input']['prev_output_tokens'][i].size(0))
            #     poisoning_loss = F.pairwise_distance(embedding[j], pre_v, p=2)
            #     total_poisoning_loss += poisoning_loss[0]
            all_poisoning_loss += total_poisoning_loss
        return all_poisoning_loss

    # def poisoning_loss(self, net_output, sample, tgt_representation=None):
    #     all_poisoning_loss = torch.tensor(0.0).cuda()
    #     for i in range(sample['id'].size):   # bacth size
    #         if sample['trigger_indexs'] is not None:
    #             trigger_index = sample['trigger_indexs'][i]
    #             # pre_v = self.choose_pre_v(trigger_index)
    #             pre_v = tgt_representation
    #         else:
    #             pre_v = tgt_representation
    #         mask_index = sample['mask_indexs'][i]
    #         embedding = net_output[1]['inner_states'][-1][:, i]
